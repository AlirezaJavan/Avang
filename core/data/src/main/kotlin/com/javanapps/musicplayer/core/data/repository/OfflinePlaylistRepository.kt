...
class OfflinePlaylistRepository
    ...
    constructor(
        ...
        override fun getPlaylists(): Flow<List<Playlist>> =
            playlistDao.getPlaylists().map { entities ->
                entities.map { it.asExternalModel() }
            }

        override fun getPlaylist(id: Long): Flow<Playlist?> = playlistDao.getPlaylist(id).map { it?.asExternalModel() }

        override fun getSongsInPlaylist(playlistId: Long): Flow<List<Song>> =
            playlistDao.getSongIdsInPlaylist(playlistId).flatMapLatest { ids ->
